...
public class BoardDao extends SuperDao {
	...
	public List<Board> selectAll() {
		...
		// 게시물 번호 역순으로 정렬
		String sql = " SELECT * FROM TEAM3_BOARD" ;
	...
	private Board getBeanData(ResultSet rs) {
		Board bean = null;
	...
	public int updateBoardData(Board bean) {
	    // bean을 사용하여 데이터베이스에 수정합니다.
	    System.out.println("updateData");
	    System.out.println(bean);

	    StringBuilder sql = new StringBuilder("UPDATE TEAM3_BOARD SET subtitle = ?, contents = ?");
	...
	private void removePreloadedFile(String attachField, int brdidx) {
	    StringBuilder sql = new StringBuilder("SELECT ").append(attachField).append(" FROM TEAM3_BOARD WHERE brdidx = ?");
	...
	public int insertBoardData(Board bean) {
		// bean을 사용하여 데이터 베이스에 추가합니다.
		System.out.println("insertData");
		System.out.println(bean);
		
		PreparedStatement pstmt = null;
		String sql = " INSERT INTO TEAM3_BOARD (memid, boardpwd, subtitle, contents, "
...
;
		String sql = " UPDATE TEAM3_BOARD SET readcnt = readcnt + 1 ";
		sql += " WHERE brdidx = ? ";
	...
	public Board getBoardDataByPk(int brdidx) {
