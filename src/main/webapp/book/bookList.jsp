...
@media ( max-width : 768px) {
	...
	.search-wrapper select, .search-wrapper input[type="text"] {
		width: 100%;
	}
	#bookSearch {
		flex-direction: column;
...
<body>
	<div class="container">
		...
		<!-- 독립적인 카테고리 선택 -->
		<form id="categoryForm" action="<%=getEnvs%>bookCategory"
			method="POST">
					...
					<c:if test="${pagelist.beginPage <= pagelist.totalPage}">
						<c:forEach var="i" begin="${pagelist.beginPage}"
							...
							<li
								...
								class="page-link"
								href="<%=getEnvs%>bookList&pageNumber=${i}&currCategory=${requestScope.selectedCategory}">${i}</a></li>
						</c:forEach>
					</c:if>
					...
					<c:if test="${pagelist.beginPage <= pagelist.totalPage}">
						<c:forEach var="i" begin="${pagelist.beginPage}"
							...
							<li
								class="page-item <c:if test="${i == pagelist.pageNumber}">active</c:if>"><a
								class="page-link"
								href="<%=getEnvs%>bookList&pageNumber=${i}&search=${requestScope.keyword}&currCategory=${requestScope.selectedCategory}">${i}</a></li>
						</c:forEach>
					</c:if>
					<c:if test="${pagelist.endPage < pagelist.totalPage}">
						<li class="page-item"><a class="page-link"
